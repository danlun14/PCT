	...
	for (int i = 0; i < 4; i++)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			...
			for (double t = 0; t <= 1; t += dt)
			{
				...
				calculate_forces_critical(p, f, m, n, x[i]); // Вычисление сил – O(N^2)
				tforces += omp_get_wtime();
				tmove -= omp_get_wtime();
...
#pragma omp atomic
					tforces -= omp_get_wtime();
					calculate_forces_atomic(p, f, m, n); // Вычисление сил – O(N^2)
#pragma omp atomic
					tforces += omp_get_wtime();
#pragma omp atomic
					tmove -= omp_get_wtime();
#pragma omp barrier
					move_particles_fornowait(p, f, v, m, n, dt); // Перемещение тел O(N)
#pragma omp atomic
					tmove += omp_get_wtime();
#pragma omp barrier
				}
...
#pragma omp atomic
					tforces -= omp_get_wtime();
					calculate_forces_nlocks(p, f, m, n); // Вычисление сил – O(N^2)
#pragma omp atomic
					tforces += omp_get_wtime();
#pragma omp atomic
					tmove -= omp_get_wtime();
...
#pragma omp atomic
					tforces -= omp_get_wtime();
					calculate_forces_fullcalc(p, f, m, n); // Вычисление сил – O(N^2)
#pragma omp atomic
					tforces += omp_get_wtime();
#pragma omp atomic
					tmove -= omp_get_wtime();
...
#pragma omp parallel num_threads(x[i])
			{
				...
				{ // Цикл по времени (модельному)
					tforces -= omp_get_wtime();
					calculate_forces_additmemory(p, f_, m, n); // Вычисление сил – O(N^2)
					tforces += omp_get_wtime();
					tmove -= omp_get_wtime();
#pragma omp barrier
					move_particles_for(p, f_, v, m, n, dt); // Перемещение тел O(N)
					tmove += omp_get_wtime();
#pragma omp barrier
				}
			}
